...
#           第四行：注释
#           数据行：属性值
#       repeated 表明下一个属性是 repeated,即数组
#           第二行: repeat 的最大次数, excel中会重复列出该属性
#           2011-11-29 做了修改 第二行如果是类型定义的话，则表明该列是 repeated
#           但是目前只支持整形
#           第三行：无用
#           第四行：注释
#           数据行：实际的重复次数
...
class SheetInterpreter:
    ...
    def _FieldDefine(self, repeated_num) :
        ...
        if field_rule == "required" or field_rule == "optional" :
            ...
            field_comment = unicode(self._sheet.cell_value(FIELD_COMMENT_ROW, self._col))  #注释

            LOG_INFO("%s|%s|%s|%s", field_rule, field_type, field_name, field_comment)

            comment = field_comment.encode("utf-8")
            self._LayoutComment(comment)  #布局注释
            ...
            if second_row.isdigit() or second_row.find(".") != -1 :
                ...
                if len(field_value_str) > 0:
                    ...
                    for field_value in field_value_list :
                        sub_field_value_list = field_value.split(";")
                        for sub_item_value in sub_field_value_list:
                            if sub_item_value!=None and sub_item_value!="":
                                if field_type == "bytes" or field_type == "string" :
                                    # item.__getattribute__(field_name).append(field_value)
                                    item.__getattribute__(field_name).append(sub_item_value)
                                else:
                                    # item.__getattribute__(field_name).append(int(float(field_value)))
                                    item.__getattribute__(field_name).append(int(float(sub_item_value)))
